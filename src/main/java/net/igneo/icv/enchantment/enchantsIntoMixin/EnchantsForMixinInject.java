package net.igneo.icv.enchantment.enchantsIntoMixin;
...
public class EnchantsForMixinInject {
    ...
    public static List<EnchantmentInstance> getChiselEnchantmentList(Level level, BlockPos tablePos, ItemStack enchItem) {
                ...
                while (i > -1) {
                    if (!bookShelf.getItem(i).isEmpty()) {
                        for (Enchantment enchantment : EnchantmentHelper.getEnchantments(bookShelf.getItem(i)).keySet()) {
                            if (EnchantsForMixinInject.checkValidEquipmentSlotFromICV(enchItem,enchantment)) {
                                boolean add = true;
        ...
        if (list.size() != localLength) {
            localEnchShift = 0;
        }
        localLength = list.size();
        return list;
    }
    ...
    public static boolean checkValidEquipmentSlotFromICV(ItemStack pStack, Enchantment enchantment){
    ...
    public static void updateEnchantmentListsDoer(){
