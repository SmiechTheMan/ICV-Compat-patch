...
import net.igneo.icv.entity.BOOST_CHARGE
...
class VolatileManager(player: Player?) :
    ...
    override fun onAttack(entity: Entity?) {
        if (player.fallDistance > 0) {
            for (entity in target?.let { collectEntitiesBox(it.level(), target!!.position(), 3.0) }!!) {
    ...
    override fun activate() {
        ...
        if (player.level() is ServerLevel) {
            val child = BOOST_CHARGE.get().create(player.level())
            child!!.owner = player
