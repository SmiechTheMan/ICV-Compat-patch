package net.igneo.icv.entity.chestplate.soulOrb

import net.igneo.icv.ICV
import net.minecraft.resources.ResourceLocation
import software.bernie.geckolib.model.GeoModel

class SoulOrbModel : GeoModel<SoulOrbEntity?>() {
    private val model = ResourceLocation(ICV.MOD_ID, "geo/placeholder.geo.json")
    private val texture = ResourceLocation(ICV.MOD_ID, "textures/entity/placeholder.png")
    private val animations = ResourceLocation(ICV.MOD_ID, "animations/placeholder.animation.json")


    override fun getModelResource(soulOrbEntity: SoulOrbEntity?): ResourceLocation {
        return this.model
    }

    override fun getTextureResource(soulOrbEntity: SoulOrbEntity?): ResourceLocation {
        return this.texture
    }

    override fun getAnimationResource(soulOrbEntity: SoulOrbEntity?): ResourceLocation {
        return this.animations
    }
}
